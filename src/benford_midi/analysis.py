...
from .utils import (
    format_p, get_first_digit, get_significand, benford_first_digit_prob,
    generate_benford_sample, z_transform, get_props
)
...
def classify_benford_compliance(test_results):
    ...
    if np.isnan(q_p):
        ...
        test_weights = {'chi2_p': 0.25, 'combined_p': 0.30, 'ks_p': 0.20, 'q_p': 0.15, 'mad': 0.10}
    ...
    if total_tests < 5:  # Small sample adjustments
        if benford_score >= 0.6 and significant_tests >= max(2, total_tests * 0.6) and mad < 0.015:
            category = "Strong"
            primary_evidence = f"High p-values ({significant_tests}/{total_tests} tests), low MAD ({mad:.4f}) [Small sample]"
        elif benford_score >= 0.3 and significant_tests >= max(1, total_tests * 0.4) and mad < 0.025:
            category = "Moderate" 
...
def process_midi_file(midi_file_path, midi_dir):
    ...
    try:
        ...
        if len(frequencies) < 10:  # Skip files with too few notes
            return None, f"Skipping {midi_file_path.name}: too few notes ({len(frequencies)})"
        ...
        test_results = (chi2_p, ks_p, q_p, m_p, g_p, combined_p, pearson_p, 
                       mad_stat, ned_stat, z_stat)
        benford_score, benford_category, primary_evidence = classify_benford_compliance(test_results)
        
        # Store results with new classification system
        file_results = [
            midi_file_path.name, chi2_p, ks_p, q_p, m_p, g_p, combined_p, pearson_p,
            mad_stat, ned_stat, z_stat, 
            benford_score, benford_category, primary_evidence, observed_props
        ]
...
def analyze_single_directory(dir_path, output_dir=None, create_plots=True):
    ...
    # Find MIDI files
    midi_files = [f for f in dir_path.iterdir() 
    ...
    with mp.Pool(processes=num_processes) as pool:
        ...
        for result, error in tqdm(results_iter, total=len(midi_files), desc="Processing MIDI files"):
    ...
    # Create DataFrame
    names = ["Name", "Chi2_P", "K-S_P", "H_Q_P", "S_M_P", "G_P", "Combined_P", "Pearson_P", 
             "MAD", "NED", "ZStat", "Benford_Score", "Benford_Category", 
             "Primary_Evidence", "Observed_Props"]
...
def compare_directories(dir1, dir2, output_dir=None, create_plots=True):
    ...
    midi_files_1 = [f for f in dir1.iterdir() 
                    if f.suffix.lower() in ('.mid', '.midi')]
    ...
    if len(midi_files_1) > 0:
        ...
        with mp.Pool(processes=num_processes) as pool:
            ...
            for result, error in tqdm(results_iter, total=len(midi_files_1), desc=f"Processing {dir1.name}"):
                if result is not None:
    ...
    midi_files_2 = [f for f in dir2.iterdir() 
                    if f.suffix.lower() in ('.mid', '.midi')]
    ...
    if len(midi_files_2) > 0:
        ...
        with mp.Pool(processes=num_processes) as pool:
            ...
            for result, error in tqdm(results_iter, total=len(midi_files_2), desc=f"Processing {dir2.name}"):
                if result is not None:
    ...
    combined_df = analyze_comparison_results(results_dir1, results_dir2, dir1.name, dir2.name)
...
def print_analysis_summary(results_df, dir_name):
    ...
    if total > 0:
        ...
        print(f"  Moderate Benford:  {moderate_count:3d} ({moderate_count/total*100:.1f}%)")
        print(f"  Weak Benford:      {weak_count:3d} ({weak_count/total*100:.1f}%)")
        print(f"  Non-Benford:       {non_benford_count:3d} ({non_benford_count/total*100:.1f}%)")
        
        # Overall compliance (Strong + Moderate + Weak)
        compliant_count = strong_count + moderate_count + weak_count
        print(f"Overall compliance: {compliant_count} ({compliant_count/total*100:.1f}%)")
...
def analyze_comparison_results(results_dir1, results_dir2, dir1_name, dir2_name):
    ...
    # Create DataFrames
    df1 = pd.DataFrame(results_dir1, columns=names) if results_dir1 else pd.DataFrame(columns=names)
    df2 = pd.DataFrame(results_dir2, columns=names) if results_dir2 else pd.DataFrame(columns=names)
...
def create_comparison_plots(combined_df, dir1_name, dir2_name, output_dir):
    ...
    compliance_rates = [
        df1[df1['Benford_Category'].isin(['Strong', 'Moderate', 'Weak'])].shape[0] / len(df1) * 100,
        df2[df2['Benford_Category'].isin(['Strong', 'Moderate', 'Weak'])].shape[0] / len(df2) * 100
    ]
    
    axes[0, 0].bar([dir1_name, dir2_name], compliance_rates, color=['skyblue', 'lightcoral'])
    axes[0, 0].set_title('Benford Compliance Rate (%)')
    axes[0, 0].set_ylabel('Percentage')
    
    # Benford score distributions
    axes[0, 1].hist(df1['Benford_Score'], alpha=0.7, label=dir1_name, bins=20, color='skyblue')
    axes[0, 1].hist(df2['Benford_Score'], alpha=0.7, label=dir2_name, bins=20, color='lightcoral')
    axes[0, 1].set_title('Benford Score Distribution')
    axes[0, 1].set_xlabel('Benford Score')
    axes[0, 1].set_ylabel('Frequency')
    axes[0, 1].legend()
...
class BenfordTests:
    def __init__(self, data):
        ...
        self.expected_counts = np.array([benford_first_digit_prob(d) * self.n for d in DIGITS])
        
        # Observed counts
        self.observed_counts = np.array([np.sum(self.first_digits == d) for d in DIGITS])
    ...
    def pearson_chi2(self):
        ...
        chi2_stat = np.sum((self.observed_counts - self.expected_counts)**2 / self.expected_counts)
    ...
    def hotelling_q(self, B=1000):
        ...
        for _ in range(B):  # Removed tqdm for multiprocessing
            ...
            q_samples.append(self.n * (test.z_bars - C) @ np.linalg.inv(sigma) @ (test.z_bars - C))
    ...
    def min_p_value_g(self, B=1000):
        ...
        p_values = np.array([(np.sum(abs_t_samples[:, d] >= abs_t[d]) + 1) / (B + 1) for d in range(9)])
        ...
        g_samples = []
        for i in range(B):
            g_samples.append(np.min([(np.sum(abs_t_samples[:, d] 
                                             >= abs_t_samples[i, d]) + 1) / (B + 1) for d in range(9)]))
    ...
    def zStat(self):
        diff_array = []
        observed_props, expected_props = get_props(self.observed_counts, self.expected_counts)
        for i in range(9):
    ...
    def NED(self):
        ...
        return np.sqrt(np.sum(np.square(observed_props-expected_props)/expected_props))
    ...
    def plot_digit_distribution(self):
        ...
        plt.bar(DIGITS - 0.2, self.observed_counts / self.n, width=0.4, label='Observed')
        plt.bar(DIGITS + 0.2, self.expected_counts / self.n, width=0.4, label='Expected (Benford)')
