...
Examples:
  ...
  %(prog)s analyze ./midi_files/pop --no_plots --output_dir ./output
    ...
    parser.add_argument(
        'directory', 
        type=str, 
        help="Path to the first MIDI directory"
    )
    ...
    try:
        if args.action == 'analyze':
            ...
            if len(results_df) > 0:
                ...
                compliant_count = len(results_df[results_df['Benford_Category'].isin(['Strong', 'Moderate', 'Weak'])])
                compliance_rate = compliant_count / len(results_df) * 100
                
                print(f"  Average Benford Score: {avg_score:.3f}")
                print(f"  Overall Compliance Rate: {compliance_rate:.1f}% ({compliant_count}/{len(results_df)} files)")
            ...
            if not args.compare_with:
                print("Error: The --compare_with argument is required for comparison.", file=sys.stderr)
                sys.exit(1)
            ...
            for i, dir_path in enumerate([midi_dir1, midi_dir2], 1):
                if not dir_path.exists():
                    print(f"Error: Directory {i} not found: {dir_path}", file=sys.stderr)
                    sys.exit(1)
                if not dir_path.is_dir():
                    print(f"Error: Path {i} is not a directory: {dir_path}", file=sys.stderr)
                    sys.exit(1)
